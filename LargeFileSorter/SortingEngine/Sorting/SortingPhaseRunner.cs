...
public class SortingPhaseRunner
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        await using IAsyncDisposable? releaseBufferSub =
            await chunksDirector.SortedLinesSaved
                ...
                .Do(async p => await logger.LogAsync(() =>
                    $"All lines in the package: {p.PackageNumber} has been saved in a file, " +
        ...
        var published = sortingPhasePoolAsObserver.StreamLinesByBatches(cancellationToken)
            ...
            .Do(async p => await logger.LogAsync(() =>
                new LogEntry(
                    $"The next chunk of data has been read. package: {p.PackageNumber}, " +
        ...
        await using var backLoopSub = await published.Select(pp => pp.Item2)
            ...
            .Do(async p => await logger.LogAsync(() =>
                $"The package: {p.PackageNumber} left a tail of {p.RemainedBytesLength} bytes; " +
                $"they will be put at the beginning of the following package."))
        ...
        await using var sortingSub = await published
            ...
            .Do(async p => await logger.LogAsync(() =>
                $"{p.LinesNumber} lines were extracted in the package: {p.PackageNumber}, " +
